    ...
    useEffect(() => {
        if (search !== '') {
            const orders = loadedOrders.filter((order: PizzaOrderHistory) => {
                return order.searchToken && order.searchToken.includes(search.toLowerCase());
            });
            setOrders(orders);
        }
        else {
                            ...
                            {orders.map((order: any) => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-1 flex-col p-4">
                                        <h3 className="mt-6 text-sm font-medium text-gray-900">{order.size?.name} Pizza</h3>
                                        <dl className="mt-1 flex flex-grow flex-col justify-between">
                                            <dt className="sr-only">Table:</dt>
                                            <dd className="text-sm text-gray-500">Table {order.table}</dd>
                                            <dt className="sr-only">Crust</dt>
                                            <dd className="text-sm text-gray-500">{order.crust?.name} Crust</dd>
                                            <dt className="sr-only">Order Date:</dt>
                                            <dd className="text-sm text-gray-500">{`Order Date: ${dayjs(order.timestamp).format('MMMM D, YYYY')}`}</dd>
                                            <dt className="sr-only">Toppings</dt>
                                            <dd className="mt-3">
                                                <span className="text-sm text-gray-500">
                                                    Toppings: {order.toppings.map((topping: any) => topping?.name).join(', ')}
                                                </span>
                                            </dd>
                                        </dl>
                                    </div>
